## 👋 Scott Alan Turner, MSDS  
**Adjunct Professor · AI Engineer · Data Scientist**

I build and teach **applied AI systems**—with a focus on *enterprise decision-making, automation, and AI agents*. My teaching philosophy is to tailor content to student's field of interest (cyber, finance, healthcare, etc.) and make it useful, practical, and relevant.

In parallel, I am the co-founder of **SportSensAI**, an AI-powered platform for athletes that improves performance and reduces injury.  
<a href="https://sportsensai.com/pitching-analysis" target="_blank" rel="noopener noreferrer">Learn more about SportSensAI</a>

---

### 🎓 Teaching (Current)
**University of Richmond – School of Professional & Continuing Studies**
- **ISYS 318U** – Artificial Intelligence & Machine Learning  
- **ISYS 312U** – Predictive Analytics  
- **ISYS 320U** – Prescriptive Analytics  

**Weatherford College**
- **ITAI 1370** – AI Fundamentals *(online & in-person)*  

**Course in Development**
- **AI Agents (Summer 2026)**  
  Applied autonomous agents, orchestration, workflows, and enterprise use cases.

---

### 🏆 Award
**Innovations in Teaching Award (2025)**  
University of Richmond — recognized for instructional innovation and applied AI pedagogy

---

### 📄 Publication
**Developing an Artificial Intelligence Course for a Small Undergraduate Program**  
ISCAP Conference (2025) — invited for second-round review for *Information Systems Education Journal (ISEDJ)*

---

### 🛠️ What I Work On
- AI agents & autonomous workflows  
- Full Stack Development  
- Start ups

**Tech Stuff:** Python · AWS · SQL · LLMs
